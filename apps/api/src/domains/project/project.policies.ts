...
export function canCreateProject(
  ...
  if (!hasAtLeastRole(membership.role, "editor")) {
    return deny(
      ...
      `role '${membership.role}' cannot create projects; 'editor' or 'owner' required`,
    );
  }
  return POLICY_ALLOW;
}
...
export function canUpdateProject(
  ...
  if (!hasAtLeastRole(membership.role, "editor")) {
    return deny(
      "PROJECT_INSUFFICIENT_ROLE",
      `role '${membership.role}' cannot update projects; 'editor' or 'owner' required`,
    );
  }
  return POLICY_ALLOW;
}

export function canChangeProjectStatus(
  membership: WorkspaceMember | null,
): PolicyResult {
...
export function canDeleteProject(
  ...
  if (membership.role !== "owner") {
    return deny(
      "PROJECT_INSUFFICIENT_ROLE",
      `role '${membership.role}' cannot archive/delete projects; 'owner' required`,
    );
  }
  return POLICY_ALLOW;
}
