...
test.describe("Client Management Flow", () => {
  test("allows creating, viewing, editing, and archiving a client", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Test Client Corp" }),
    ).toBeVisible();
    ...
    await page
      .getByLabel(/Client \/ Contact Name/i)
      .fill("Test Client Updated");
