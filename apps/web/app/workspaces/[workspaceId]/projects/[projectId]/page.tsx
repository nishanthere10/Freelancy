...
interface ProjectDetailPageProps {
  params?: Promise<{ workspaceId: string; projectId: string }> | { workspaceId: string; projectId: string };
}
...
export default function ProjectDetailPage({ params }: ProjectDetailPageProps) {
  ...
  const workspaceId =
    resolvedParams?.workspaceId || (routeParams?.workspaceId as string);
  const projectId =
    resolvedParams?.projectId || (routeParams?.projectId as string);
      ...
      <div className="max-w-[1400px] w-full mx-auto px-6 sm:px-10 lg:px-12 py-10 sm:py-14 lg:py-16 pb-24">
        {isLoading ? (
          ...
          <div className="max-w-md mx-auto text-center py-16 px-6 bg-white rounded-[var(--radius-xl)] border border-[var(--color-hairline-soft)] shadow-sm space-y-4">
            ...
            <div>
              <h2 className="text-base font-bold text-[var(--color-ink-deep)]">Project Not Found</h2>
              <p className="text-xs text-[var(--color-slate-text)] mt-1">
                {error instanceof Error ? error.message : 'This project does not exist or has been removed.'}
              </p>
            </div>
            <Button variant="secondary" onClick={handleBack} className="rounded-full shadow-xs mx-auto">
