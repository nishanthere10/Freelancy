...
describe("DriftAnalysisModal", () => {
  ...
  it("renders initial input state when open", () => {
    ...
    expect(
      screen.getByText(
        /Evaluating change request against: SaaS Invoicing App/i,
      ),
    ).toBeDefined();
    expect(
      screen.getByLabelText(/Client Change Request \/ Message/i),
    ).toBeDefined();
    expect(
      screen.getByRole("button", { name: /analyze scope drift/i }),
    ).toBeDefined();
  });
